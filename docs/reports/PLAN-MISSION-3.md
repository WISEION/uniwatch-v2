...
**Статус:** ЧЕРНОВИК, **вероятно вытеснен** `TENDER_INTELLIGENCE_SPEC.md` §6 (Phase 3 — Supply Capability Graph), см. `docs/decisions/OPEN-QUESTIONS.md` (2026-08-04) — новый документ переносит реальный (не synthetic-first) supply-side ingestion в Phase 3. Файл не удалён (как synthetic-контур для тестирования движка SCG он может остаться актуальным частично) — не активируется до явного сравнения с новым документом на старте Phase 3. Активируется только после GO супервайзера по Exit gate Phase 2 (PLAN-MISSION-2.md). Подготовлен заранее по прямому запросу владельца («till last phase») — планирование всех оставшихся фаз, исполнение остаётся строго последовательным.
**Дата:** 2026-08-04
