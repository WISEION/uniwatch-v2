...
- `apps/api_tender/routers/health.py` and `apps/api_vendor/routers/health.py` — readiness put the raw driver error text in an unauthenticated response body and kept no log. Logged instead; the body says only `database unreachable` (client-visible change). The vendor copy of the router landed on `master` (ADR-0006) while this branch was open and carried the same defect, so it was fixed in the merge; the two routers' loggers are now named per service (`uniwatch.api_tender.health`, `uniwatch.api_vendor.health`).
...
- `packages/platform/pagination.py` + `apps/api_tender/routers/admin_users.py` — a malformed `cursor` raised a raw base64/JSON error and came back as `internal_error` 500. `decode_cursor` raises typed `InvalidCursor`; the route answers 400 `invalid_cursor` (client-visible change, `FR-PLT-05`).
