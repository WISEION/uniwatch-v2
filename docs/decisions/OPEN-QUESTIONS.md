...
## 2026-08-16 — Task 6.B (Gate 3/4/5 wiring, backup/restore drill)

**Context:** `PLAN-MISSION-6.md` §3 task 6.B — Gate 3 (release candidate), Gate 4 (production authorization), Gate 5 (post-deploy verification) from master plan §22, plus the `NFR-REL-01` restore drill. Built directly by this session plus five parallel subagents, each independently briefed and reviewed. Exploration before writing the plan found one previously-undocumented real gap: `docs/adr/0005-authority-model.md`'s "production deployment requires a distinct approver from the initiator" (`FR-AUT-06`/`INV-14`) had **zero mechanical enforcement** anywhere — the repo's GitHub ruleset only requires `Fast gate`/`Full gate` status checks, no required-reviewer rule, so every prior PR's distinct-approver fact was a manually-asserted `WORKLOG.md` note, not something CI or GitHub verified.

**Deviation/assumption — deployment authorization (Gate 4, built directly, the real gap above):**
1. `scripts/authorize_deployment.py` is the first mechanical check of `ADR-0005`'s distinct-approver rule: takes a PR number, reads the PR's real head commit and author via `gh api`, requires an explicit `--approver` distinct from that author, checks `release-manifest.json`'s recorded commit against the PR's head commit, checks the target DB's schema version against `EXPECTED_SCHEMA_VERSION`, then writes an append-only `deployment_authorizations` row (migration `0022`, schema version 21→22). Refuses (non-zero exit, no DB write) on any check failing.
2. **A real bug found and fixed while verifying this against PR #39's actual data**: `ci.yml`'s `build-images` job (task 6.A) tagged images and stamped `release-manifest.json` with bare `github.sha` — but on a `pull_request`-triggered run, `github.sha` is GitHub's own ephemeral test-merge commit, which matches neither the PR's real head commit nor its eventual `merge_commit_sha`. PR #39's own manifest proved this: its recorded `commit_sha` (`ef7c16b...`) matched neither `head.sha` (`10193f8...`) nor `merge_commit_sha` (`9a62ae4...`) — the "commit↔digest proof" Gate 4 exists to check was silently unverifiable for every PR-triggered build so far. Fixed to `github.event.pull_request.head.sha || github.sha` (real on both triggers); `authorize_deployment.py`'s own comparison uses the PR's `head.sha` exclusively (not `merge_commit_sha`, since CI builds from the branch's head, before any merge commit exists).
3. Verified end-to-end against real data, not fixtures: ran `authorize_deployment.py` against the real, already-merged PR #39 with a manifest matching its true head commit — confirmed it refuses when approver==initiator (`accessunico`/`accessunico`), refuses on a mismatched commit, and succeeds with the real distinct identities (`accessunico`/`WISEION`), writing a real row to a verification DB.
4. "Kill switches" (Gate 4's other line item) — cited from Phase 5's already-tested `policy_store.kill_switch`, no new code; it is policy-graph-specific, and no general job/worker emergency-stop exists or was invented.

**Deviation/assumption — backup/restore drill (Gate 3, `NFR-REL-01`, subagent):**
1. `scripts/backup.py`/`scripts/restore.py` shell out to real `pg_dump -Fc`/`pg_restore`, fail loudly (never a silent partial file) on a missing binary or non-zero exit. `tests/integration/test_restore_drill.py` is the actual drill evidence: seeds real rows (RBAC + a policy graph + a raw snapshot), backs up, restores into a **second, separate, never-migrated database** on the same Postgres instance, asserts every row survives byte-for-byte — proving `pg_restore` alone reconstructs schema+data from the dump, not merely that the tools run.
2. **A second real bug found during verification, fixed directly**: neither subprocess call passed `stdin=subprocess.DEVNULL`, so `pg_dump`/`pg_restore` could inherit the caller's stdin and hang indefinitely (e.g. on a libpq prompt) instead of failing fast — the whole point of this module's fail-loud contract. Fixed on both scripts.
3. **Local-machine-only finding, not fixed (out of scope to alter the user's system PATH):** this dev machine has PostgreSQL 18 client tools on `PATH`, while the test's testcontainer is `postgres:16-alpine` — PG18's `pg_dump` emits a `SET transaction_timeout` directive (PG17+) that PG16's `pg_restore` doesn't recognize, so a local run fails on that version skew. CI installs `postgresql-client` via `apt-get` on `ubuntu-latest` (Ubuntu 24.04's default is v16, matching the test container), so this specific failure is not expected to reproduce there — flagged, not silently worked around.

**Deviation/assumption — live DB invariant checker (Gate 5, subagent):**
1. `packages/platform/invariant_checks.py` (three checks: FK-orphan on a `notifications`-style table, a read-only confirmation that `policy_versions_one_active_per_graph`'s structural guarantee holds, FK-orphan on `users.role_id`) + `scripts/check_invariants.py` (CLI, pass/fail per check, non-zero exit on any failure) — the reusable, live-DB entrypoint `test_invariant_quarantine.py`'s ad-hoc migration-hook function never provided.
2. **Notable finding**: no migration actually creates a `notifications` table — it only ever existed as `test_invariant_quarantine.py`'s own tmp-path fixture, built to exercise the generic preflight/postflight mechanism, not a real schema object. Rather than crash (`UndefinedTable`) against the real schema, the check first confirms via `to_regclass` whether the table exists at all, and reports `passed=True` with an explicit "vacuously satisfied, not actually checked" detail if not — never a silent false-positive pass.
3. Every check's negative case was proven, not assumed: a seeded orphaned row, a deliberately-dropped structural index, and a genuine FK-constraint drop (looked up dynamically via `pg_constraint`, not assumed by naming convention) each independently confirmed to make the relevant check fail with an actionable detail.

**Deviation/assumption — real-endpoint smoke test by role (Gate 5, subagent):**
1. `scripts/smoke_test.py` (real HTTP, not in-process ASGI — the first such test in this repo) + `scripts/seed_smoke_test_users.py` (idempotent, clearly-labeled throw-away seed accounts — chosen over requiring an operator's own pre-existing accounts, since a fresh deployment's DB has none, and proving deny-by-default RBAC needs a role denied on one route and allowed on another, which no arbitrary existing account is guaranteed to exercise).
2. Verified against a real running `uvicorn` process + real migrated Postgres (not the full `docker-compose.local.yml` stack, per the task's own carve-out for this machine's confirmed Docker-networking unreliability) — full login→allowed-route→denied-route→logout→post-logout-401 lifecycle passed for both seeded roles; a deliberately wrong password was confirmed to produce a real `FAIL` line and non-zero exit rather than a silent pass.

**Deviation/assumption — dependency/license/SBOM scanning (Gate 3, `NFR-SEC-07`, subagent):**
1. `pip-audit`/`pip-licenses`/`cyclonedx-bom` added as dev dependencies; new `ci.yml` `security-scan` job (`needs: fast`). Dependency scan is **non-blocking** (real findings exist against this repo's current pinned versions — `pip`/`pytest`/`starlette` — with no established clean baseline yet, matching `.ci/README.md`'s own already-recorded rollout plan); license scan is a **real blocking gate from day one** (no "clean baseline" ambiguity for a binary license-allowlist check) and was verified to genuinely pass against this repo's actual resolved dependencies; SBOM (CycloneDX JSON) is informational.
2. `chardet` (LGPL-2.1, a transitive dependency of `cyclonedx-bom` itself, confirmed via `pip show`) is excluded from the license allow-list check via `--ignore-packages` rather than added to the allow-list — never imported by product code, never present in a production image (`apps/*/Dockerfile` install without the `dev` extra) — recorded as a distribution-scope judgment call to revisit if it ever became a direct/production dependency, not a silent override.

**Deviation/assumption — operational runbook + release-notes template (Gate 3, subagent):**
1. `docs/operations/runbook.md` sequences all of the above (backup → restore-drill currency check → invariant check → `authorize_deployment.py` → deploy → smoke test → fall back to `docs/operations/cutover-plan.md`'s rollback on failure) — written concurrently with the mechanisms it documents, referencing each by planned name/purpose since their final shape wasn't fixed yet at write time.
2. `docs/operations/release-notes-template.md` is a template only — no release has happened yet — with sections referencing `docs/reports/WORKLOG.md` (detailed history) and the `deployment_authorizations` record (who authorized) rather than duplicating either.

**Source conflict (if any):** None.

**Owner follow-up needed:** No decision needed to consider 6.B's mechanism-building complete. Two honest local-environment gaps recorded above (PG18/PG16 client-version skew; this machine's general Docker-networking flakiness, already tracked) are not expected to block CI. `D-PILOT`/`D-SLO` remain open, unaffected by this task. Task 6.C (observability — parallel to 6.B, independent module) or 6.D (pilot ops, needs `D-PILOT`) are the next tasks in `PLAN-MISSION-6.md` §3's order.
...
## 2026-08-17 — D-PILOT resolved; Task 6.D (pilot users + permission matrix) started

**Context:** `PLAN-MISSION-6.md` §5 defines `D-PILOT` as "пилотные пользователи и точная permission matrix" — the one decision blocking task 6.D specifically (not 6.A-6.C, already done). Owner answered directly in chat: 12 pilot users across 4 roles — 6 `worker`, 2 `tender`, 2 `procurement`, 2 `technical_specialist` — then confirmed "make it so all roles be asigned", i.e. provision all 12 accounts now rather than staging through a manual bootstrap-one-admin flow.

**Deviations/assumptions:**
1. **The exact permission-to-role mapping (which of the system's existing permissions each of the 4 named roles gets) was proposed by this session, not dictated verbatim by the owner** — the owner named the 4 roles and headcounts; this session mapped each role to the real, already-enforced permissions in `apps/api_tender/routers/*.py` (grep'd, not guessed) based on the role names' plain meaning: `worker` = broad read + execution-facts logging, `tender` = drives bid/no-bid decisions + forecasting, `procurement` = owns post-decision outcomes/execution close-out, `technical_specialist` = owns policy graph config (`algorithm.*`) + user administration (`admin.users.*`). This is the same class of decision as task 6.A's "5 consecutive failures locks the account" — a reasonable engineering default for an underspecified detail, not an invented `D-*`/`TBD-*` number — but unlike that case, the exact mapping should be reviewed against real job duties once the 12 real people are identified, not assumed permanently correct.
2. **`technical_specialist` combines `algorithm.*` and `admin.users.*` into one role** (2 people) — a deliberate breadth choice given only 2 pilot users in that bucket, not a case of "no all-access role" (hard ban #7): the role still has an explicit, bounded permission list, not every permission in the system (e.g. it has no `decision.*` permissions at all).
3. **Pilot user accounts are real, not throwaway smoke-test accounts** (contrast `scripts/seed_smoke_test_users.py`) — `scripts/seed_pilot_users.py` generates a fresh random password per account with `secrets.token_urlsafe`, never hardcodes one, and prints each exactly once to stdout. Usernames are placeholders (`pilot_<role>_<n>`) since no real individual names were given — rename via `display_name` (`PATCH /admin/users/{id}`, `admin.users.update`) once real identities are assigned to each slot; the endpoint does not support renaming `username` itself.
4. **The seed script is idempotent for roles/permissions but deliberately NOT idempotent for already-created users** — re-running it never resets an already-distributed real password or role, unlike the smoke-test script's intentional always-reset behavior. Changing an existing pilot user's role after the fact goes through `PATCH /admin/users/{id}`, not a script re-run.
5. **Verified live, not just unit-tested** (no test file exists for this script, matching `scripts/seed_smoke_test_users.py`/`scripts/check_alerts.py`'s own established precedent of leaving thin one-shot operator scripts untested): ran the script against a real local Postgres, creating all 12 accounts; confirmed via real HTTP against a running `api_tender` that a `pilot_worker_1` login is denied (`403`) on `GET /admin/users` while `pilot_technical_specialist_1` is allowed (`200`) — the deny-by-default boundary between roles is real, not asserted.
6. **Training materials, feedback queue, and the go-live/rollback decision pack** (`PLAN-MISSION-6.md` §3 task 6.D's other two line items) are not addressed by this entry — this entry covers only the first line item ("pilot users and the exact permission matrix"). Recorded as a gap, not silently skipped.

**Source conflict (if any):** None — this is the owner directly answering the question `PLAN-MISSION-6.md` itself posed for `D-PILOT`.

**Owner follow-up needed:** Real individual names/identities for the 12 placeholder pilot slots, and the generated passwords (already handed over once at seed time — not recoverable from this document or the database) — nothing further needed to consider the permission-matrix half of task 6.D closed. Task 6.D's remaining two line items (training materials/feedback queue, go-live/rollback decision pack) remain open.

## 2026-08-18 — Task 6.D's remaining line items: feedback queue, onboarding doc, go-live pack template

**Context:** `PLAN-MISSION-6.md` §3 task 6.D's other two line items, continuing directly from the D-PILOT entry above: "training materials and feedback queue for pilot users" and "go-live/rollback decision pack".

**Deviations/assumptions:**
1. **New permission `platform.feedback.submit` was granted to all 4 pilot roles, `platform.feedback.triage` to `technical_specialist` only** — added to `scripts/seed_pilot_users.py`'s `PILOT_ROLES` matrix and re-run against the already-seeded roles (idempotent for role_permissions, confirmed live: re-running left all 12 existing user rows untouched while adding the new grants). Verified live in a real browser: `pilot_worker_1` was denied (`403`) before the re-run and succeeded (`201`) immediately after, with no re-login needed (`get_current_identity` re-resolves permissions fresh from the DB on every request, not cached at login).
2. **No dedicated triage UI was built in `apps/web`** — `GET /pilot-feedback` and `POST /pilot-feedback/{id}/resolve` (both `platform.feedback.triage`) are API-only for now. A `technical_specialist` triages via direct API calls. Recorded as an honest gap in `docs/operations/pilot-onboarding.md`, not silently built partway or hidden.
3. **"Training materials"** is interpreted as `docs/operations/pilot-onboarding.md` — a real onboarding doc (login flow, per-role capability table, how to submit feedback) rather than a training video/session, which this session cannot produce. If synchronous training turns out to be required, that is a new, separate deliverable.
4. **The go-live/rollback decision pack (`docs/operations/go-live-decision-pack.md`) is a template, not a filled-in decision** — same "template only, no real content until the real event happens" discipline as `docs/operations/release-notes-template.md`. It does not fabricate shadow-comparison results, restore-drill dates, or user-acceptance data; each section points at the real mechanism to pull evidence from (`packages/tender/shadow_comparison.py`, `packages/platform/restore_drill.py::latest_passing_drill`, `packages/platform/pilot_feedback.py`) and is explicit about which of those mechanisms (shadow-comparison run history) doesn't durably exist yet.
5. **`docs/operations/README.md` was still the Phase-0 stub with zero real links** (a gap the 6.C final review already flagged and left unfixed, M8) — fixed here as part of this same docs pass, listing every real file now in `docs/operations/`.
6. Migration `0024_pilot_feedback.sql` bumps schema version 23→24; every hardcoded `23` reference (`settings.py`'s dev default, the two health-endpoint tests, `test_migrations_runner.py`'s five assertions) was updated in the same commit, same discipline as task 6.C's migration 0023.

**Source conflict (if any):** None.

**Owner follow-up needed:** No decision needed to consider task 6.D's three line items mechanism-complete (pilot users + permission matrix, feedback queue + onboarding doc, go-live pack template). What remains genuinely open, not buildable by more code: a real pilot needs to actually run before `go-live-decision-pack.md` can be filled in with real evidence, and `packages/tender/shadow_comparison.py`'s missing run-history table (assumption 2 of the 2026-08-17 task 6.C entry) would need to be built before that section of the pack could be automated rather than hand-filled. Task 6.E (security/qa, closes Phase 6's exit gate) is next per `PLAN-MISSION-6.md` §3.
