...
| Image digest integrity | A deployed artifact must match the exact commit that was reviewed (v1 RN-11/RN-12 failure mode: tag drift) | Immutable image digest recorded at build, checked at production authorization against the reviewing commit | Production-authorization gate (`INV-14`), Phase 6 — the *build* half (compute + record each image's digest) is now wired: `ci.yml`'s `build-images` job, `docs/operations/release-manifest.md`. The *check-at-authorization* half is still Phase 6 task 6.B, not yet wired. |

None of the above was implemented in this task (0.C) — this table was the plan `0.D` (and later Phase 6 task 6.A, for the image-digest row) wired against, per `docs/reports/PLAN-MISSION-1.md` §2 ("CI security-gate stub ... план"). Full inclusion criteria and tool selection are recorded here (not silently decided) the moment each check is actually wired.
