...
If `fetch_page` or `ingest_bom_lines_page` raises, this function raises too,
*before* calling `store.checkpoint` -- so the caller's job-processing loop
(see `apps/worker/main.py`'s `_process_claimed_job` for the reference
shape) never advances the cursor for a page that didn't durably commit,
and a retry resumes at the exact same page (FR-JOB-04/05). A brand new job
